...
public class DataUtil {
    ...
    public static void exportToExcel(String fileName, String tabTitle, List<ExcelBean> totalData, List<List<String>> urls, StringBuilder errorPage) {
        ...
        try (ExcelWriter excelWriter = ExcelUtil.getWriter(fileName, "查询结果")) {
            ...
            excelWriter.addHeaderAlias("country", "国家");
            excelWriter.addHeaderAlias("region", "区域");
            excelWriter.addHeaderAlias("city", "城市");
            ...
            excelWriter.merge(9 + config.additionalField.size(), tabTitle, true);
    ...
    public static List<? extends BaseBean> loadJsonData(TabDataBean bean,
        ...
        FofaConfig config = FofaConfig.getInstance();
        if (!isExport && bean != null) {
            bean.count = (bean.page - 1) * Integer.parseInt(config.getSize());
        }
        ArrayList<String> fields = config.additionalField;
        for(int index=0; index < array.size(); index ++){
            ...
            String country = _array.getString(8);
            String region = _array.getString(9);
            String city = _array.getString(10);

            // 创建一个Map来存储所有附加字段的值
            HashMap<String, String> additionalData = new HashMap<>();
            for (int i = 0; i < fields.size(); i++) {
                String fieldName = fields.get(i);
                String fieldValue = _array.getString(11 + i); // 基础字段有11个
                additionalData.put(fieldName, fieldValue);
            }
            ...
            if(isExport){ // 是否为导出数据
                ...
                d.setRegion(region);
                d.setCity(city);
                d.setLastupdatetime(additionalData.get("lastupdatetime"));
                d.setFid(additionalData.get("fid"));
                d.setOs(additionalData.get("os"));
                d.setProduct(additionalData.get("product"));
                d.setIcp(additionalData.get("icp"));
                d.setCerts_subject_cn(additionalData.get("certs_subject_cn"));
                d.setCerts_subject_org(additionalData.get("certs_subject_org"));
                ...
                b.setRegion(region);
                b.setCity(city);
                b.setFid(additionalData.get("fid"));
                b.setIcp(additionalData.get("icp"));
                b.setOs(additionalData.get("os"));
                b.setCertCN(additionalData.get("certs_subject_cn"));
                b.setProduct(additionalData.get("product"));
                b.setCertOrg(additionalData.get("certs_subject_org"));
                b.lastupdatetime = additionalData.get("lastupdatetime");
                if(list.contains(b)){
                    TableBean b2 = list.get(list.indexOf(b));
                    if(port == 443 || port == 80){
                        // host 带有额外的 443 或 80
                        if(b2.host.contains(":" + port)){
                            b.num = b2.num;
                            list.remove(b2);
                        }else if(b.host.contains(":" + port)){
                            continue;
                        }
                    }
                    // host 相同时 去掉不带title的
                    if(b2.host.equals(b.host)){
                        if(b2.title != null && !b2.title.isEmpty()){
                            continue;
                        }else{
                            b.num = b2.num;
                            list.remove(b2);
    ...
    public static Double getValueFromIP(String ip){
        if(ip.contains(":")){ // a simple check for ipv6
